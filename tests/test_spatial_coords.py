...
def check_spatial_coords(ds1: xr.Dataset, ds2: xr.Dataset, verbose=False):
    ...
    if spatial_dims_1 != spatial_dims_2:
        if verbose: 
            ...
            print("The spatial dimensions are not the same." + Style.RESET_ALL)
            print(f"Dataset 1 spatial dims: {spatial_dims_1}")
            print(f"Dataset 2 spatial dims: {spatial_dims_2}\n")
        return False
    ...
    for dim in spatial_dims_1:
        if ds1[dim].shape != ds2[dim].shape:
            if verbose: 
                print(Fore.CYAN + f"Spatial Coordinates Err Output: ")
                print(f"Comparing majority opinion {get_filename(ds1)} with {get_filename(ds2)}")
                print("The spatial dims do not have the same shape" + Style.RESET_ALL)
                print("The shape of {dim} in dataset 1 is {ds1[dim].shape}")
                print("The shape of {dim} in dataset 2 is {ds2[dim].shape}\n")
            return False
